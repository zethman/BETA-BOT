...
export class RiskManager {
  ...
  private pendingExposureSol(): number {
    let sum = 0;
    for (const size of this.pendingOpens.values()) sum += size;
    return sum;
  }
  ...
  canOpenPosition(mint: string, sizeSol: number): string | null {
    ...
    if (this.pendingOpens.has(mint)) return `buy already pending for ${mint}`;
    return null;
  }

  /** Atomically reserve capacity before awaiting an RPC submission. */
  reserveOpen(mint: string, sizeSol: number): string | null {
  ...
  releaseOpenReservation(mint: string) {
